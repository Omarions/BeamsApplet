...
public class BeamsControlPanel extends JPanel {
    ...
    private class ButtonListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent event) {
            ...
            //check the clicked button, for start button, start the timer
            if (action == start) {
                beamsPanel.resume();
            }
            //check the clicked button, for stop button, stop the timer.
            if (action == stop) {
                beamsPanel.suspend();
            }
            //check the clicked button, for colorButton, display color dialog and set the color the beams to the choosen color.
            if (action == colorButton) {
                color = chooser.showDialog(null, "Change beam color", color);
                beamsPanel.changeColor(color);
                //start new thread to update GUI.
                UpdateGUIThread updateGUIThread = new UpdateGUIThread(beamsPanel);
    ...
    private class SpeedListener implements ChangeListener {
        ...
        public void stateChanged(ChangeEvent event) {
            //set the speed or the delay of timer from the slider.
            beamsPanel.setDelay(((JSlider) event.getSource()).getValue());
            //start new thread to update GUI.
            UpdateGUIThread updateGUIThread = new UpdateGUIThread(beamsPanel);
            updateGUIThread.start();
    ...
    private class BeamListener implements ChangeListener {
        ...
        @Override
        public void stateChanged(ChangeEvent event) {
            //set the beams number from the slider.
            beamsPanel.setBeamNumber(((JSlider) event.getSource()).getValue());
